...
IIT_list = "|c00bfffffinfo [] toggle [] allmembers []|r" ..IIT_outputlist
...
function InterruptSay:COMBAT_LOG_EVENT_UNFILTERED(...)
	...
	if InterruptSayDB.intsayonoff==1 then
		if aEvent=="SPELL_INTERRUPT" then
			...
			if aUser~=UnitName("player") then
				intsaymsg = ("=> '"..aUser.."'的"..GetSpellLink(userSpellID).."打断了'"..destName.. "'的"..GetSpellLink(destSpellID))
			else
				intsaymsg = ("=> 我的"..GetSpellLink(userSpellID).."打断了'"..destName.. "'的" ..GetSpellLink(destSpellID))
			end
				
			if InterruptSayDB.INTSAYOUTPUT=='Emote' then
				if aUser~=UnitName("player") then return end
				SendChatMessage("打断了'"..destName.."'的"..GetSpellLink(destSpellID), "EMOTE")
